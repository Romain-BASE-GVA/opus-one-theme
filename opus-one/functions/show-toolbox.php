...
function get_show_from_category($term_id){
    global $wpdb;
    $my_shows = array();
    $rows = $wpdb->get_results("SELECT DISTINCT P.ID, M.meta_value FROM wp_posts P, wp_postmeta M, wp_term_relationships T WHERE (P.ID = M.post_id AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_la_representation' AND M.meta_value >= '".date('Ymd')."' AND M.meta_value NOT LIKE 'field%' AND T.term_taxonomy_id = ".$term_id." AND T.object_id = P.ID) OR (P.ID = M.post_id AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_report' AND M.meta_value >= '".date('Ymd')."' AND M.meta_value NOT LIKE 'field%' AND T.term_taxonomy_id = ".$term_id." AND T.object_id = P.ID) ORDER BY meta_value ASC", ARRAY_A);
    return $rows;
}

function get_first_show_category($term_id){
    global $wpdb;
    $rows = $wpdb->get_results("SELECT DISTINCT ID, meta_value FROM wp_posts P, wp_postmeta M, wp_term_relationships T WHERE P.ID = M.post_id AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_la_representation' AND M.meta_value >= '".date('Ymd')."' AND M.meta_value NOT LIKE 'field%' AND T.term_taxonomy_id = ".$term_id." AND T.object_id = P.ID ORDER BY meta_value ASC LIMIT 1", ARRAY_A);
    return $rows[0];
}
...
{
    global $wpdb;
    $rows = $wpdb->get_results("SELECT DISTINCT ID, meta_value FROM wp_posts P, wp_postmeta M, wp_term_relationships T WHERE (P.ID = M.post_id AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_la_representation' AND M.meta_value >= '".date('Ymd')."' AND M.meta_value NOT LIKE 'field%' AND T.term_taxonomy_id = ".$term_id." AND T.object_id = P.ID) OR (P.ID = M.post_id AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_report' AND M.meta_value >= '".date('Ymd')."' AND M.meta_value NOT LIKE 'field%' AND T.term_taxonomy_id = ".$term_id." AND T.object_id = P.ID) ORDER BY meta_value DESC LIMIT 1", ARRAY_A);
...
function get_next_show_of_month_by_term($month, $year, $term_id){
    ...
    $rows = $wpdb->get_results("
                    ...
                    FROM wp_posts P, wp_postmeta M, wp_term_relationships T 
...
function get_show_from_category_nb($term_id, $nb, $post_id){
    global $wpdb;
    $rows = $wpdb->get_results("SELECT DISTINCT P.ID, M.meta_value FROM wp_posts P, wp_postmeta M, wp_term_relationships T WHERE (P.ID = M.post_id AND M.post_id != '".$post_id."' AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_la_representation' AND M.meta_value >= '".date('Ymd')."' AND M.meta_value NOT LIKE 'field%' AND T.term_taxonomy_id = ".$term_id." AND T.object_id = P.ID) OR (P.ID = M.post_id AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_report' AND M.meta_value >= '".date('Ymd')."' AND M.meta_value NOT LIKE 'field%' AND T.term_taxonomy_id = ".$term_id." AND T.object_id = P.ID) ORDER BY meta_value ASC", ARRAY_A);
...
function get_next_show_of_month($date){
    global $wpdb;
    $rows = $wpdb->get_results("SELECT DISTINCT ID, meta_value FROM wp_posts P, wp_postmeta M WHERE (P.ID = M.post_id AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_la_representation' AND (M.meta_value LIKE '".$date."%' OR M.meta_value <= '".$date."%') AND M.meta_value >= '".date('Ymd')."') ORDER BY meta_value ASC", ARRAY_A);
...
function get_first_show(){
    global $wpdb;
    $rows = $wpdb->get_results("SELECT DISTINCT ID, meta_value FROM wp_posts P, wp_postmeta M WHERE P.ID = M.post_id AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_la_representation' AND M.meta_value >= '".date('Ymd')."' AND M.meta_value NOT LIKE 'field%' ORDER BY meta_value ASC LIMIT 1", ARRAY_A);
    return $rows[0];
}
...
function get_next_shows_terms($nb, $post)
{
    ...
    $rows = $wpdb->get_results("
                SELECT DISTINCT P.ID, M.meta_value 
                FROM wp_posts P, wp_postmeta M, wp_postmeta Q, wp_postmeta T
...
{
    global $wpdb;
    $rows = $wpdb->get_results("SELECT DISTINCT ID, meta_value FROM wp_posts P, wp_postmeta M WHERE (P.ID = M.post_id AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_la_representation' AND M.meta_value >= '".date('Ymd')."' AND M.meta_value NOT LIKE 'field%') OR (P.ID = M.post_id AND P.post_status = 'publish' AND M.meta_key LIKE '%_date_de_report' AND M.meta_value >= '".date('Ymd')."' AND M.meta_value NOT LIKE 'field%') ORDER BY meta_value DESC LIMIT 1", ARRAY_A);
    return $rows[0]['meta_value'];
}
