...
$swp_query = new WP_Query(
    array(
        ...
        'meta_query' => array(
            array(
                'key' => 'afficher_dans_les_resultats_de_recherche',
                'value' => 'non',
                'compare' => '!=',
            ),
        )
...
$i = 0;
foreach ($swp_query->posts as $show)
{
    $i += 1;
    $date = get_next_show($show->ID);
    if(!$date){
        $swp_query->posts = array_splice($swp_query->posts, $i, 1);
    }
}
?>
...
<div data-barba="container" data-barba-namespace="agenda" data-bg="#6E32FF" data-text-color="#fff"
    ...
    <main class="main main--agenda page-template-page-agenda">
        ...
        if (!empty($swp_query->posts)) {
            $first_show = $swp_query->posts[0];
            $date = get_next_show($first_show->ID);
            $next_shows = array_splice($swp_query->posts, 1, 1);
            $date_show = $date['date_de_la_representation'];
            $year_to_show = substr($date_show, 0, 4);
            $month_to_show = substr($date_show, 4, 2);
            $month = date_i18n("F", strtotime($date_show));
        ...
        <ul class="event-month" data-month-name="<?php echo ucfirst($month); ?>"
            ...
            <ul class="event-list">
                ...
                $the_id = $show->ID;
                $date = get_next_show($the_id);
                $date = $date['date_de_la_representation'];
        ...
        <ul class="event-month" data-month-name="<?php echo ucfirst($month); ?>"
            ...
            <ul class="event-list"><?php
                ...
                if ($post) {
                    ...
                    /* @TODO : Prévoir un affichage en cas de non resultat */
